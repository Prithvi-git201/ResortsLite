...
public class BookingService {
    ...
    // Authentication credentials are now stored in AWS Secrets Manager
    @Value("${aws.secrets.auth.name:resorts-auth-credentials}")
     ...
     */
    private String getAuthCredentials(String userId) {
        try {
            GetSecretValueRequest getSecretValueRequest = GetSecretValueRequest.builder()
            ...
            JsonNode secretJson = objectMapper.readTree(secret);
            return secretJson.get(userId) != null ? secretJson.get(userId).asText() : null;
        } catch (Exception e) {
            throw new RuntimeException("Failed to retrieve authentication credentials from AWS Secrets Manager", e);
        }
    }
    ...
    public Map<String, Object> createBooking(String guestName, String roomType,
        ...
        // FIXED cr-java-0069: Database host is now retrieved from AWS Secrets Manager
